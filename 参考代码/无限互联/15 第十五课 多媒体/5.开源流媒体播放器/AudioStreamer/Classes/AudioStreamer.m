...
- (void)startInternal
{
	...
	do
	{
		...
		//重新连接
//		if (downloadState == NO && ++reConnectionNumber <= 10) {
//			//[self openReadStream];
//			[self httpResume];
//			downloadState = YES;
//		}
...
- (void)start
{
	@synchronized (self)
	{
		...
		else if (state == AS_INITIALIZED || state == AS_BUFFERING)
		{
...
- (void)stop
{
    NSString *path = [NSHomeDirectory() stringByAppendingPathComponent:@"Documents/1.mp3"];//[NSString stringWithFormat:@"%@"]; ;
...
//		[self failWithErrorCode:AS_AUDIO_DATA_NOT_FOUND];
	...
	else if (eventType == kCFStreamEventHasBytesAvailable)
	{
        ...
        [fileData appendBytes:bytes length:sizeof(bytes)];
