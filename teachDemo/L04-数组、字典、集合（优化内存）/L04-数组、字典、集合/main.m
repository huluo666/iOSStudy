...
#pragma mark - 3.将所有学员放入一个数组;
    ...
    for (int i = 0; i < 5; i++) {
        ...
        Student *student = [[Student alloc] initWithInfoDictionary:infoDictionary]; // 1
        [students addObject:student]; // 2
        [student release];
        
        // 持有对象的方法：
        // 通过指向对象的指针，来持有对象
        // 集合类可以持有加入集合中的对象（retain一次），当集合对象自身销毁的时候，会将自身中的所有对象release一次
        // 持有集合类，不会增加集合内部对象的引用计数值
    }
...
//    NSInteger age = [manager agesBetweenStudentsWithIdentity:[identityList objectAtIndex:1]
